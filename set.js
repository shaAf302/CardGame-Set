...
function init1(mod_val){
    
    var mod = mod_val ;
    modt = mod_val;
    if(mod == 10)
    {
        difficulty = "Practice Mod";
    }
    else
    {
        difficulty = "Competitive Mod"
    }

    var gameSelection = document.getElementById("game-selection1");
    gameSelection.classList.add("hide");

}

function init(players_num){
    players = players_num;
    ...
    document.getElementById("mod").innerHTML=difficulty;
...
function dealNewBoard() {
    ...
    if (countSets(on_table) == 0) {
        ...
        notification.innerHTML = "cards delt";
        var newBoard =[0,2,4,7,9,11];
        replaceCards(newBoard);
        notification.style.color = "black";
    }
    checkRemaining();
}
...
function checkRemaining() {
    if (deck.length + on_table.length < 21) {
        ...
        if (countSets(remaining) == 0) {
            ...
            if(nofp == 1)
            {
                  notification.innerHTML = "You have selected " + setValue + " sets ! restart game";
            }
            else
            {
                if(p1set>p2set)
                {
                    notification.innerHTML = "Player 1 has won with " + p1set + " sets";
                }
                if(p1set<p2set)
                {
                    notification.innerHTML = "Player 2 has won with " + p2set + " sets";
                }
                else
                {
                    notification.innerHTML = "Match draw with " + p1set + " sets";
                }
